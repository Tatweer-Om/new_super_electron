...
@section('main')
    @push('title')
        <title>{{ trans('messages.inspection_setting_lang', [], session('locale')) }}</title>
    @endpush
            ...
            <div class="page-header settings-pg-header">
                <div class="add-item d-flex">
                    <div class="page-title">
                        <h4> {{ trans('messages.inspection_agrrement_lang', [], session('locale')) }}</h4>
                        <h6> {{ trans('messages.write_terms_lang', [], session('locale')) }}</h6>
                    </div>
                </div>
                ...
                <div class="col-xl-12">
                    <div class="settings-wrapper d-flex">
                                    ...
                                    <ul>
                                        <li class="submenu-open">
                                            <ul>
                                                ...
                                                <li class="submenu">
                                                    <a href="javascript:void(0);"class="active subdrop"><i data-feather="archive"></i><span>
                                                        {{ trans('messages.maint_agree_lang', [], session('locale')) }}</span><span class="menu-arrow"></span></a>
                                                    <ul>
                                                        <li><a href="{{ url('inspection_setting') }}"class="active">{{ trans('messages.inspection_setting_lang', [], session('locale')) }}</a></li>
                        ...
                        <div class="settings-page-wrap">
                            <form action="" method="Post" id="inspection_data">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <textarea id="bulletTextarea"  class="form-control" style="resize: both; overflow: auto;" name="inspect">{{ $inspect->inspection_detail ?? '' }}</textarea>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="text-end settings-bottom-btn">
                                    <button type="button" class="btn btn-cancel me-2">Cancel {{ trans('messages.cancel_lang', [], session('locale')) }}</button>
                                    <button type="submit" class="btn btn-submit">{{ trans('messages.save_lang', [], session('locale')) }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
